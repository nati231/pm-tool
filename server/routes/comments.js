...
module.exports = function commentRoutes(db) {
  ...
  router.post('/', async (req, res) => {
    try {
      ...
      const isMember = await assertMember(task.projectId, req.userId);
  ...
  router.get('/task/:taskId', async (req, res) => {
    try {
      const task = await db.orm.public.Task.where({ id: req.params.taskId }).first();
      if (!task) return res.status(404).json({ error: 'Task not found' });

      const isMember = await assertMember(task.projectId, req.userId);
      if (!isMember) return res.status(403).json({ error: 'Not a member of this project' });

      const comments = await db.orm.public.Comment.where({ taskId: req.params.taskId }).all();

      // sort oldest first
      comments.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));

      // attach author name/email to each comment (separate lookups, cautious pattern)
      const withAuthors = await Promise.all(
        comments.map(async (c) => {
          const author = await db.orm.public.User.where({ id: c.authorId }).first();
          return {
            ...c,
